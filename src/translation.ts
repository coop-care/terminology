...
export module Translation {
  ...
  export function estimateTranslationCosts(source: any, sourceLang: string, targetLang: string): string {
    let textCount = 0;
    ...
    return [textCount, "Texte,", characterCount, "Zeichen,", 
  ...
  function traverseObject(object: any, callback: (value: string, path: string[]) => void, path?: string[]) {
  ...
  export function translate(source: any, target: any, sourceLang: string, targetLang: string) {
    traverseObject(source, function(target) {
